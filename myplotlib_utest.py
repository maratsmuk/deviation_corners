...
class MyTestCase(unittest.TestCase):
    def test_generated_files(self):
        ...
        df = df[df.columns[:4]]
        json_name = 'deviation_shortened.json'
        ...
        self.assertEqual(len(list(paths_corr))+len(list(paths_box))+len(list(paths_scatter))+len(list(paths_hist))+len(list(paths_his_all)), len(list(newpl.plot_paths)),
                         'Error: lengths of lists do not coincide')
        self.assertEqual(1+9+4+3+2,
